...
export class DBTService {
     ...
     */
    async findRelatedModels(searchTerms) {
        ...
        for (const model of allModels) {
            ...
            if (relevanceScore > 0) {
                relatedModels.push({ ...model, score: relevanceScore });
            }
        }
     ...
     * Run DBT commands
     */
    async runDBTCommand(command, options = {}) {
     ...
     */
    extractDependencies(sqlContent) {
        const dependencies = [];
        // Extract ref() calls
     ...
     */
    async parseSchemaFile(schemaFile, models) {
        ...
        for (const modelSchema of schema.models) {
            ...
            if (model) {
                ...
                if (modelSchema.columns) {
                    model.columns = modelSchema.columns.map((col) => ({
                        name: col.name,
                        type: col.data_type,
                        description: col.description,
                        tests: col.tests ? col.tests.map((test) => typeof test === 'string' ? test : test.test || Object.keys(test)[0]) : []
                    }));
                }
            }
        }
     ...
     * Enhance models with manifest data
     */
    enhanceModelsWithManifest(models, manifest) {
        if (!manifest.nodes)
            return;
        for (const model of models) {
            // Find corresponding node in manifest
            const nodeKey = Object.keys(manifest.nodes).find(key => key.includes(model.name) && manifest.nodes[key].resource_type === 'model');
            if (nodeKey) {
                const node = manifest.nodes[nodeKey];
                // Update dependencies
                if (node.depends_on && node.depends_on.nodes) {
                    model.dependencies = node.depends_on.nodes.map((dep) => {
                ...
                if (node.columns && !model.columns?.length) {
                    model.columns = Object.values(node.columns).map((col) => ({
                        name: col.name,
                        type: col.data_type,
                        description: col.description
                    }));
     ...
     */
    async annotateModel(modelName, annotations) {
        ...
        let modelEntry = schema.models.find((m) => m.name === modelName);
        if (!modelEntry) {
        ...
        if (annotations.columns) {
            ...
            annotations.columns.forEach(newCol => {
                let existingCol = modelEntry.columns.find((c) => c.name === newCol.name);
                if (!existingCol) {
