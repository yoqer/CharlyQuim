...
import { CLIConfig, SQLEdit } from '../types/index.js';

export class CLICommands {
	private ragEngine: VoidRAGEngine;
	 ...
	 */
	async testSnowflake(): Promise<void> {
	...
	private async displayContext(
		...
		if (ragResults.snippets.length > 0) {
			...
			ragResults.snippets.slice(0, 3).forEach((snippet: any, index: number) => {
		...
		if (dbtModels.length > 0) {
			...
			dbtModels.slice(0, 5).forEach((model: any) => {
	 ...
	 */
	private async displayEdits(edits: SQLEdit[]): Promise<void> {
		console.log(chalk.yellow('\n📝 Proposed Changes:'));
	 ...
	 */
	private async executeAction(action: string, edits: SQLEdit[], intent: string): Promise<void> {
		const spinner = ora();
	 ...
	 */
	private async saveEditsToFiles(edits: SQLEdit[]): Promise<void> {
	 ...
	 * Update DBT documentation
	 */
	private async updateDBTDocumentation(edits: SQLEdit[]): Promise<void> {
		...
		for (const edit of modelFiles) {
			try {
				...
				const model = Array.from((this.dbtService as any).modelsCache.values())
					.find((m: any) => m.name === modelName);

				if (model) {
					const docs = await this.llmService.generateModelDocumentation(model as any, sqlContent);
