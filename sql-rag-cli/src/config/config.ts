...
export class ConfigManager {
	 ...
	 */
	private async loadFromFile(filePath: string): Promise<CLIConfig> {
		try {
	 ...
	 */
	private loadFromEnvironment(): CLIConfig {
		return {
			llm: {
	 ...
	 */
	private mergeWithEnvironment(fileConfig: Partial<CLIConfig>): CLIConfig {
		const envConfig = this.loadFromEnvironment();

		return {
			llm: {
				provider: (process.env.LLM_PROVIDER as 'openai' | 'anthropic') || fileConfig.llm?.provider || envConfig.llm.provider,
	...
	private validateConfig(config: CLIConfig): void {
		...
		requiredSnowflakeFields.forEach(field => {
			if (!config.snowflake[field as keyof typeof config.snowflake]) {
	 ...
	 */
	async createExampleConfig(outputPath: string = '.sql-rag.json'): Promise<void> {
		const exampleConfig: Partial<CLIConfig> = {
			llm: {
				provider: 'openai',
				model: 'gpt-4',
