...
export class LLMService {
	 ...
	 */
	async explainChanges(edits: SQLEdit[]): Promise<string> {
		try {
...
Wrap your response in \`\`\`json and \`\`\` tags.`;
	 ...
	 */
	private buildExplanationPrompt(edits: SQLEdit[]): string {
	 ...
	 */
	private buildDocumentationPrompt(model: DBTModel, sqlContent?: string): string {
		...
		prompt += `\nProvide a clear description for the model and descriptions for each column. Format as JSON:
{
  ...
  "columnDescriptions": [
	 ...
	 */
	private buildImprovementPrompt(sqlCode: string, issues?: string[]): string {
		...
		prompt += `Please provide improved code and explain the improvements. Format as JSON:
{
  ...
  "improvements": ["list of improvements made"]
	 ...
	 */
	private parseEditResponse(response: string, contextSnippets: CodeSnippet[]): SQLEdit[] {
		try {
	 ...
	 */
	private parseImprovementResponse(response: string): {
		improvedCode: string;
		improvements: string[];
	} {
		try {
