...
export class MetricsMainService extends Disposable implements IMetricsService {
	...
	private get oldId() {
		...
		const newKey = 'orcide.app.oldMachineId'
		...
		const oldValue = this._appStorage.get('orcide.machineId', StorageScope.APPLICATION) ?? 'NULL' // the old way of getting the key
		...
		// private get oldId() {
		// 	return this._memoStorage('orcide.app.oldMachineId', StorageTarget.MACHINE, 'NULL')
		// }
	}
	...
	private get distinctId() {
		...
		return this._memoStorage('orcide.app.machineId', StorageTarget.MACHINE, setValIfNotExist)
	}

	// just to see if there are ever multiple machineIDs per userID (instead of this, we should just track by the user's email)
	private get userId() {
		return this._memoStorage('orcide.app.userMachineId', StorageTarget.USER)
	}
